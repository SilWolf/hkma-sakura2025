...
const recalculateStatistics = async (matchId: string) => {
  // 1. 獲取聯賽資料、玩家資料、隊伍資料
  const { tournament, playersMap, teamsMap } =
    await apiGetTournamentByMatchId(matchId);
...
export async function PATCH(request: Request) {
  ...
  if (response.success) {
    // Do heavy calculations on statistics. Do it async.
    recalculateStatistics(match._id);
  }
